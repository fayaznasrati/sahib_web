...
class HotelRoomAndHall extends Model
{
    ...
    protected $fillable = [
        ...
        public function DeleteProduct($id){
            ...
            //   dd($id);
        $pro=HotelRoomAndHall::findOrFail($id);

        if (File::exists("service-brand/hotel-room-cover/".$pro->cover)) {
            File::delete("service-brand/hotel-room-cover/".$pro->cover);
        }
        $images=HotelRoomAndHallImage::where("hotel_room_and_hall_id",$pro->id)->get();
        foreach($images as $image){
        if (File::exists("service-brand/hotel-room-images/".$image->image)) {
            File::delete("service-brand/hotel-room-images/".$image->image);
        }
        }
        $pro->delete();
