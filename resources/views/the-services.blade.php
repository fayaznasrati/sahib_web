    ...
    <div class="mobile-design">
        ...
        <div class="scrolling-wrapper">
            @foreach ($services as $service)
                <div class="my_card"><a href="/service/brand-info//{{ $service->slug }}/{{ $service->slug }}">
                        <img  src="../../service-brand/logo/{{ $service->logo }}" alt="asdf" class="my_card__image" style="max-height:200px;object-fit: cover"/>
                        <div class="card__overlay_desc">
                            <div class="pro_title">
                                <a href="/service/brand-info/{{$service->slug}}" style="font-size:11px;">
                                    <label style="color:green;">{{ strlen($service->brand_name) > 19 ? substr($service->brand_name, 0, 19) . '...' : $service->brand_name }}</label>
                                    <span style="color:rgb(117, 22, 9)">
                        ...
                        @foreach ($services as $service)
                            <div class="col-xs-6 col-sm-6 col-md-3 col-lg-3 col-6 ">
                                <div class="card" style="margin-bottom:20px;"> 
                                    <img src="../../service-brand/logo/{{ $service->logo }}" alt="{{$service->logo}}" style="max-height: 100px;object-fit: cover;">
                                    <div class="container m-1" style="text-align: left;">
                                        <a href="/service/brand-info/{{$service->slug}}" style="font-size:11px;">
                                            <label style="color:green;">{{ strlen($service->brand_name) > 19 ? substr($service->brand_name, 0, 19) . '...' : $service->brand_name }}</label>                  </a><br>
                                        <span style="color:rgb(117, 22, 9)">
                                            <strong>Services</strong>
                                        </span><br>
                                        <span style="font-size: 10px" >{{$service->address}}</span>
